...
namespace t24e::local_mapper::post_processing {
    ...
    class Filtering {
        ...
        public:
            ...
            static float entropyRow(float* row, std::size_t size);

            /*! \brief Calculate the maximum for a single row. */
            static float maxRow(float* row, std::size_t size);
            ...
            */
            static std::pair<float*,std::size_t> filter(float* predictions, std::vector<int64_t> tensorDims,
            ...
            */
            static std::vector<cnn::bounding_box_t> nmsIoU(float* predictions, std::vector<int64_t> tensorDims,
